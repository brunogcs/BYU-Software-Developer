...
def pressure_gain_from_water_height(height):
    rho = 998.2       # densidade da água
    g = 9.80665       # gravidade
    P = (rho * g * height) / 1000  # resultado
    return P
...
def reynolds_number(hydraulic_diameter, fluid_velocity):
    rho = 998.2       # densidade da água 
    mu = 0.0010016    # viscosidade dinâmica da água
...
def pressure_loss_from_pipe_reduction(larger_diameter, fluid_velocity, reynolds_number, smaller_diameter):
    # Parâmetros
    rho = 998.2  # kg/m^3 (água)
    D = larger_diameter
    d = smaller_diameter
    R = reynolds_number
    v = fluid_velocity

    # k conforme a imagem: (0.1 + 50/R) * ((D/d)^4 - 1)
    k = (0.1 + 50.0 / R) * ((D / d)**4 - 1.0)

    # P em kPa: -k * rho * v^2 / 2000
    P_kPa = -(k * rho * v**2) / 2000.0
    return P_kPa
